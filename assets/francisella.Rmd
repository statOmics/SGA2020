...
In this file we will fetch the data from the github repository linked to the SGA2019 course: https://statomics.github.io/SGA2019.
...
library(tidyverse)
library(limma)
peptidesFile <- "~/Downloads/SGA2019-data/quantification/francisella/peptides.txt"
proteinGroupsFile <-"~/Downloads/SGA2019-data/quantification/francisella/proteinGroups.txt"
...
```{r}
pepData<-selectFeatureData(pepData,fcol=c("Proteins","Contaminant","Reverse","Sequence"))
```
...
The information on the spike in genotype is available as the first letter of the sample name.
We extract this information and recast the character vector in a factor.

```{r}
pData(pepData)$genotype <- pepData %>% sampleNames %>% substr(2,3) %>% as.factor
```
...
Indeed the samples do not seem to be clearly separated according to the spike in genotype.
...
We notice that the leading differences (log FC) in the protein data is still according to technical variation. On the second dimension, however, we also observe a clear separation according to the spike-in genotype. Hence, the summarization that accounts for peptide specific effects makes the effects due to the spike-in genotype more prominent!

```{r}
plotMDS(exprs(protData),col=as.double(pData(pepData)$genotype))
```
...
We only have to model the data using the factor genotype from the pheno data of the protein level MSnSet.
...
So we specify it at c(0,0) to indicate that the intercept (first 0) and the parameters for the factor genotype (second 0) are not penalized.
...
models <- fit.model(protdata=protMSqRob, response="quant_value", fixed="genotype",shrinkage.fixed=c(0,0),robust_var=FALSE)
```
...
```{r}
L <- makeContrast("genotypeD8 - genotypeWT",  levels=c("genotypeD8","genotypeWT"))
...
plotDataStack<-data.frame(quant_value=c(plotDat),sequence=rep(rownames(plotDat),rep=ncol(plotDat)),sample=rep(colnames(plotDat),each=nrow(plotDat)),genotype=rep(pData(pepData)$genotype,each=nrow(plotDat)))
plot1 <- ggplot(plotDataStack, aes(x=sample, y=quant_value,fill=genotype))
