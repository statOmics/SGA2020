...
#Data Import
##Load data
mzid files can be load via de mzID function in the mzID package.
Because we prefer to work with a the data in a table format we flatten the data.
We show the first 6 rows of data.

```{r}
library(mzID)
dataFlat <- flatten(mzID("~/Downloads/SGA2019-data/identification/pyroSwissprot.mzid"))
head(dataFlat)
```
...
```{r}
data <- dataFlat[,c("isdecoy","x!tandem:expect")]
names(data) <- c("decoy","score")
data <- na.exclude(data)
data$score <- -log10(as.double(data$score))
```
...
Note, that the target decoy assumption is violated. We explained the reasons for this in the class.
