...
\item Note, that $n_c$ equals the number of contrasts, and
\item $\mb{L}$ is assumed to be full rank
\end{itemize}
...
\begin{itemize}
\item i.e. some of the contrasts can be written as linear combinations of other contrasts,
\item the inverse of the variance covariance matrix of the contrasts does not exist
\item we can then decompose L in $r$ orthogonal contrasts $\mb{Q}$ with
\[Q_j^T Q_k^T = \delta_{jk},\]
\item $j,k \in 1,\ldots,r$,
\item $\delta{jk} = 0$ and if $j\neq k$ $\delta{jk} = 1$
\item which can be done using the QR decomposition  
\item We can that assess the omnibus null hypothesis using the statistic:
