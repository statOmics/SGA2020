...
We also know that a chi-square distribution with one degree of freedom is equivalent to the square of a normal distribution.
So we can easily convert the chi-square test statistic in a z statistic which should be normally distributed under the null.

The normal distribution is very useful for diagnostics because we are used to work with it. If we have two sided test statistics we can always convert the results into a z-statistic.

1. We can start of the two-sided p-values.
2. We devide the p-values by two to have the probability in one of the tails
...
\[
FDR = E[lfdr] \approx \frac{\sum\limits_{k \in DE-set} lfdr_k}{n_DE}\]
...
library(locfdr)
ttAll$table <- ttAll$table %>%
  mutate(lfdr = locfdr(z)$lfdr)
```

In the plot we see the histogram of the z-values.
The green line is the estimate of the marginal distribution $f(z)$.
The blue line is the estimate of the null distribution.
We see that null distribution appears to be much broader than the standard normal.

We see that the marginal distribution (green distribution) is not estimated well. We also get a warning message that indicates that we can increase the degrees of freedom 
